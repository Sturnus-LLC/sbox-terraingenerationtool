...
public static class IslandShapes
{
	public static float DefaultIsland( int x, int y, int width, int height, long seed, bool warp, float warpSize = 0.1f, float warpStrength = 0.5f )
	{
		...
		float falloff = 1.0f - Math.Clamp( distance, 0, 1 ); // Smooth taper from center to edge
		...
		float beachFalloff = Math.Clamp( (distance - beachStart) / (beachEnd - beachStart), 0, 1 );
		float beachTaper = (1.0f - beachFalloff) * 0.1f; // Smooth transition to flat region
